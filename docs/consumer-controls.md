...
- **Wrappers (`workflow_call`)**: expose `review-reference-file` only together with `review-reference-source`, and lock the wrapper's default to `base` so product repos cannot opt back to workspace mode. This matches the "Expose with `review-reference-source: base` locked in the wrapper" category in the wrapper input-surface guidance — see [Extending the wrapper's input surface](../README.md#extending-the-wrappers-input-surface).
