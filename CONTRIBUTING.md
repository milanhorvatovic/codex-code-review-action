...
The bundled Agent Skill at [`skills/codex-review/`](skills/codex-review/) follows the [Skill System Foundry](https://github.com/milanhorvatovic/skill-system-foundry) layout: a router skill (`SKILL.md`), one capability per subdirectory under `capabilities/<name>/capability.md`, and all executable code under `scripts/` at the skill root. Internal modules live under `scripts/lib/`. The skill is implemented in **Python 3.12+ (stdlib only)** so it ships without `npm install`, `pip install`, or any other dependency setup — just Python on `PATH`.

New capabilities follow the same conventions:

1. **Markdown for prompts, Python for glue.** A capability is a `capabilities/<name>/capability.md` that names the entry-point script under `scripts/<name>.py`. The capability prompt drives the LLM; the script performs deterministic work (file emission, validation, diff rendering). The split keeps emitted YAML byte-stable across model temperatures and lets `unittest` call the entry-point function directly without invoking a live agent runtime.
2. **Compose internals, do not duplicate them.** Detection, schema mapping, invariants assertion, reference-file layering, pin resolution, and findings parsing live under `skills/codex-review/scripts/lib/`. If a capability needs a fact those internals do not yet expose, extend the relevant module — never inline detection logic in an entry-point script.
3. **Walk the consumer-controls invariants before you write.** Any script that emits or modifies a workflow file must call `assert_workflow()` from `scripts/lib/invariants/` and refuse to write on failure. The invariant IDs (`CC-01..CC-09`, plus `CC-EXTRA-01-bare-action`) are encoded in [`skills/codex-review/references/invariants.md`](skills/codex-review/references/invariants.md) and cross-checked against [`docs/consumer-controls.md`](docs/consumer-controls.md) by `scripts/test_invariants_consistency.py`.
4. **Resolve the action pin at runtime.** Use `resolve_pin()` from `scripts/lib/pin_resolver.py`; it shells out to `gh api releases/latest`. Do not maintain a static pin table — `scripts/refresh-self-pins.ts` already covers `README.md` and the dogfood workflow, and adding another verifier would duplicate state.
5. **Add a fixture before adding a capability.** Drop the consumer-side input files under `tests/skills/codex-review/__fixtures__/<name>/` (or a conformant `findings.json` under `__fixtures__/findings-examples/`) and exercise the capability's entry-point function against the fixture in a `unittest` test case under `tests/skills/codex-review/`. Tests and fixtures live OUTSIDE the skill directory because skill installers copy the skill source tree verbatim — anything physically inside `skills/codex-review/` ships to integrators.
6. **Stdlib only.** No `pip install`. No third-party packages. The whole point of the Python rewrite is that integrators can use the skill without any dependency setup. Stick to `json`, `re`, `pathlib`, `subprocess`, `argparse`, `unittest`, and `dataclasses`.
7. **CLIs.** Each entry script ships a `main(argv=None) -> int` that parses `argparse` arguments and exits with `sys.exit(main())`. Capabilities reference the script as `python3 scripts/<name>.py --help`; report-producing scripts should also support `--json` for non-interactive agent runtimes.
8. **Run tests locally.** From the repo root: `PYTHONPATH=skills/codex-review/scripts python3 -m unittest discover -s tests/skills/codex-review -p "test_*.py" -v`. The `tests-skills.yaml` workflow runs the same command plus a `--help` smoke test on every PR, and verifies that no test files have leaked back into the skill directory.
