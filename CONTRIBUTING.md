...
5. **Add a fixture before adding a capability.** Drop the consumer-side input files under `skills/codex-review/__fixtures__/<name>/` (or a conformant `findings.json` under `__fixtures__/findings-examples/`) and exercise the capability's entry-point function against the fixture in a `unittest` test case alongside the script. This is how tests stay portable across CI environments.
...
8. **Run tests locally.** From `skills/codex-review/scripts/`: `python3 -m unittest discover -p "test_*.py" -v`. The `tests-skills.yaml` workflow runs the same command plus a `--help` smoke test on every PR.
