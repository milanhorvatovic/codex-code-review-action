...
The bundled Claude Code skill at [`skills/codex-review/`](skills/codex-review/) is a router with a small primitives layer (`lib/`) and one capability per subdirectory under `capabilities/`. The skill is implemented in **Python 3.12+ (stdlib only)** so it ships without `npm install`, `pip install`, or any other dependency setup — just Python on `PATH`.
...
2. **Split prompt from glue.** Each capability ships a `capability.md` that drives the LLM and a `run.py` that performs deterministic work (file emission, validation, diff rendering). The split keeps emitted YAML byte-stable across model temperatures and lets `unittest` call `run.py` directly without invoking Claude Code.
3. **Walk the consumer-controls invariants before you write.** Any capability that emits or modifies a workflow file must call `assert_workflow()` from `lib/invariants/` and refuse to write on failure. The invariant IDs (`CC-01..CC-09`, plus `CC-EXTRA-01-bare-action`) are encoded in [`skills/codex-review/references/invariants.md`](skills/codex-review/references/invariants.md) and cross-checked against [`docs/consumer-controls.md`](docs/consumer-controls.md) by `references/test_invariants_consistency.py`.
4. **Resolve the action pin at runtime.** Use `resolve_pin()` from `lib/pin_resolver.py`; it shells out to `gh api releases/latest`. Do not maintain a static pin table — `scripts/refresh-self-pins.ts` already covers `README.md` and the dogfood workflow, and adding another verifier would duplicate state.
5. **Add a fixture before adding a capability.** Drop the consumer-side input files under `skills/codex-review/__fixtures__/repos/<name>/` (or a conformant `findings.json` under `__fixtures__/findings-examples/`) and exercise the capability against the fixture in a `unittest` test case. This is how golden tests stay portable across CI environments.
6. **Stdlib only.** No `pip install`. No third-party packages. The whole point of the Python rewrite is that integrators can use the skill without any dependency setup. Stick to `json`, `re`, `pathlib`, `subprocess`, `unittest`, and `dataclasses`.
7. **Run tests locally.** From the skill directory: `python3 -m unittest discover -p "test_*.py" -v`. The `tests-skills.yaml` workflow runs the same command in CI on every PR.
