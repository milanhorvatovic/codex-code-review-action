...
- changes the validation, resolution, or sourcing of `review-reference-file` or `review-reference-source` (both inputs validated under [`src/prepare/referenceFile.ts`](src/prepare/referenceFile.ts); base-mode resolution sources the file at the PR base SHA via `git show`). A change that *adds new content paths into the prompt itself* belongs in the trust-boundary class above; a change to *how an existing input is validated or sourced* belongs here.
