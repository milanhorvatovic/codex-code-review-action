...
- changes the validation, resolution, or sourcing of `review-reference-file` or `review-reference-source`. The `review-reference-source` value is parsed and validated in [`src/config/inputs.ts`](src/config/inputs.ts); the path-shape rules and per-mode resolvers live in [`src/prepare/referenceFile.ts`](src/prepare/referenceFile.ts); base-mode resolution sources the file at the PR base SHA via `git ls-tree --literal-pathspecs` (entry mode) and `git cat-file blob` (content), with `git cat-file -s` enforcing the size cap before any content is buffered. A change that *adds new content paths into the prompt itself* belongs in the trust-boundary class above; a change to *how an existing input is validated or sourced* belongs here.
...
5. Squash-merge the self-pin refresh PR. (Skipped for pre-releases — `release.yaml` does not open a refresh PR for `vX.Y.Z-rc.N`.)
   - **5b. Attach the gate evidence zip after all post-tag items are verified.** Wait until every post-tag gate item — including any release-specific post-tag items the maintainer recorded under [Release-specific items](docs/release-gate.md#release-specific-items) — has been signed off. For final releases the self-pin refresh PR merge in step 5 is part of this set; for pre-releases (`vX.Y.Z-rc.N`) the refresh PR is intentionally skipped, so the gate proceeds as soon as the GitHub Release exists and any RC-specific post-tag items are signed off. Then package the filled gate (`gate.md`), captured validation output (`validation.log`, `dist-diff.txt`, `audit.json`), and any manual-check notes (`manual-checks.md`) into `release-gate-v<X.Y.Z>.zip` and upload to the GitHub Release: `gh release upload v<X.Y.Z> release-gate-v<X.Y.Z>.zip --clobber`. The `--clobber` flag overwrites an existing asset of the same name so retries (partial-upload recovery, evidence regeneration after a late post-tag verification) stay idempotent. See [Archiving the gate](docs/release-gate.md#archiving-the-gate).
...
7. Squash-merge the self-pin refresh PR. (Skipped for pre-releases — the release workflow does not open a refresh PR for `vX.Y.Z-rc.N`.)
   - **7b. Attach the gate evidence zip after all post-tag items are verified.** Wait until every post-tag gate item — including any release-specific post-tag items recorded under [Release-specific items](docs/release-gate.md#release-specific-items) — has been signed off. For final releases the self-pin refresh PR merge in step 7 is part of this set; for pre-releases (`vX.Y.Z-rc.N`) the refresh PR is intentionally skipped, so the gate proceeds as soon as the GitHub Release exists and any RC-specific post-tag items are signed off. Then package the filled gate plus captured validation output and manual-check notes into `release-gate-v<X.Y.Z>.zip` and upload to the GitHub Release: `gh release upload v<X.Y.Z> release-gate-v<X.Y.Z>.zip --clobber`. The `--clobber` flag overwrites an existing asset of the same name so retries (e.g. regenerating the zip after a late post-tag verification) stay idempotent. See [Archiving the gate](docs/release-gate.md#archiving-the-gate).
