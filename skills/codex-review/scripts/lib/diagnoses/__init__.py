...
Findings shape to a Diagnosis. The capability runs every diagnosis and
renders the firing ones with their unified diffs and rationales.
"""
...
from .types import Diagnosis, DiagnosisFn, DiagnosisKind, Recommendation, RecommendationTarget
...
def run_all_diagnoses(findings: Findings) -> tuple[Diagnosis, ...]:
    return tuple(diagnose(findings) for diagnose in ALL_DIAGNOSES)
