...
export function buildSignoffSection(gateDocUrl: string = resolveGateDocUrl()): string {
  const lines: string[] = [
    ...
    "- [ ] Base-mode regression checks for `review-reference-source: base` are run against the merge candidate (the integration suite covers them via `npm test`; the scratch-PR exercise per [Base-mode regression checks](" + gateDocUrl + "#base-mode-regression-checks) is recommended for the evidence zip).",
      ...
      switch (plan.mode) {
        case "fresh":
          ...
          if (
            ...
            findSignoffSectionStart(existingBody) >= 0 &&
            !existingBody.includes(SIGNOFF_TEMPLATE_VERSION_MARKER)
          ) {
            // The previous body had a sign-off heading but lacked the
            // current template-version marker. existingBodyHasMaintainerEdits
            // returned false (no Verified-by / Waived / checked-box signal
            // and no unknown-lines because the version marker gates that
            // backstop), so we replaced the section with the current
            // template. If the maintainer had begun adding free-form notes
            // or release-specific table rows to the older section without
            // those structured signals, those edits were just overwritten.
            // The trade-off is documented on existingBodyHasMaintainerEdits;
            // surface it here so the maintainer can recover any lost
            // content from the PR's edit history.
            stdoutWrite(
              `WARNING: the previous PR body had a sign-off section generated by an older template (no ${SIGNOFF_TEMPLATE_VERSION_MARKER} marker) and no detected sign-off signal (Verified by: / Waived: / checked box). The body has been replaced with the current template. If you had added free-form notes or release-specific table rows to the older section, recover them from the PR's edit history (the GitHub UI 'edited' indicator) and re-add them — preferably with a checkbox or Verified-by line so future reruns preserve them.\n`,
            );
          }
          break;
        case "merge":
