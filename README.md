...
- *"Does the action maintainer see the diff?"* — **No.** No maintainer-operated destination exists. The action's source is auditable in this repository, and `openai/codex-action` is SHA-pinned in [`review/action.yaml`](review/action.yaml) (currently `@c300d2798ce5b59a7667c806b5e3a3d2c397ed2f`, v1.7) so the referenced commit is immutable unless this repo bumps the SHA. (Runtime behavior of OpenAI's API and model selection are outside this guarantee — see OpenAI's data-handling terms.)
...
> - **Tamper resistance (opt-in via `review-reference-source: base`).** The constraints above do not pin policy to the base branch. In the default `workspace` mode the reference file is checked out from `${{ github.event.pull_request.head.sha }}`, so a same-repo PR can legitimately edit `.github/codex/review-reference.md` and steer the review prompt. Setting `review-reference-source: base` reads the file from the PR base SHA via `git ls-tree` (for the entry mode) and `git cat-file blob` (for the content) instead, so an in-flight PR cannot edit policy that applies to its own review. Recommended for repositories with less-trusted contributors and as the locked setting in org-owned wrapper workflows.
>
...
| `base` (recommended for less-trusted contributors) | The PR base SHA, via `git ls-tree` (entry mode) and `git cat-file blob` (content). | Policy is locked at the base commit, so an in-flight PR cannot edit policy that applies to its own review. Cost: you cannot land a policy update and reference it in the same PR — the new policy takes effect on PRs opened after the policy commit lands on base. |

Recommend `base` for repositories with less-trusted contributors and as the hard-coded setting in org-owned wrapper workflows.
...
- **Expose only with `review-reference-source: base` hard-coded in the wrapped step.** Reference-policy knobs decide what the model treats as review policy. If the wrapper exposes `review-reference-file`, **hard-code** `review-reference-source: base` in the wrapped `prepare` step's `with:` block — do **not** expose `review-reference-source` as a `workflow_call` input with a `base` default, because reusable-workflow inputs are caller-controlled and a product repo can override the default back to `workspace`. Base mode pins the *source* of the policy file to the consumer repo's base SHA: an in-flight PR cannot edit `.github/codex/review-reference.md` and have those edits apply to its own review. Policy *content* still lives in the consumer repo and is controlled by whoever can land changes to base — base mode protects against in-PR edits, not against trusted maintainers merging policy changes through normal review. The wrapper hard-codes the mode choice (product repos cannot opt back to workspace), not the policy text. If the wrapper does need to expose the source mode for some narrow reason, validate the input and reject any value other than `base` before invoking `prepare`. Exposing `review-reference-file` without one of these two patterns is not recommended: in workspace mode the policy file is read from the PR head, so any same-repo PR in a product repo can edit the policy and steer its own review (see [the production-example callout above](#production-workflow-example)).
