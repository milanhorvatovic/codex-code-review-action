...
The `review-reference-file` input names a tracked path inside the consumer repository. Whether the file is read from the PR head (workspace mode) or the PR base SHA (base mode) is governed by `review-reference-source` — see [Safe `review-reference-source` modes](#safe-review-reference-source-modes) below. The path-shape rules below apply to every value of `review-reference-source`. They prevent a PR from coercing the prepare step into reading runner-local files outside the workspace (e.g. `/proc/self/environ`) or runner-managed git state (e.g. `.git/config`) and forwarding their contents to OpenAI in the prompt:
...
- `base` (recommended for less-trusted contributors and as the default in org-owned wrapper workflows) — the file is read at the PR base SHA via `git show`. The git form does not follow filesystem symlinks, so a PR cannot point the read at a runner-local secret by replacing the file with a symlink in the workspace. In addition to the shared path-shape rules, base mode rejects:
